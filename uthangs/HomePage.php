...
//   $result2 = $conn->query($sql2);
  $sql = "SELECT d_id, d_name, phone, address, DATE(created_at) AS date from debtors;";
      ...
      <div class='customer-box-lists'>
          ...
          <div>
              ...
              while($row1 = $result2->fetch_assoc()) {
                ...
                </div><a href='forgot.php? &bid=".$row["d_id"]." &bname=".$row["d_name"]."&address=".$row["address"]."&phone=".$row["phone"]. "&credate=" .$row["date"]."' class='confirm-btn'>Edit</a>
                <a href=deletedebtor.php?bid=" .$row["d_id"]. " &bname=".$row["d_name"]." ' class='confirm-btn'>Delete </a> 
      </div>";}
	 }
  } else {
	echo "0 results";
